...
export default function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith('/api')) {
    ...
    if (hasSqlInjection(fullUrl)) {
      console.warn('[proxy] SQL Injection attempt blocked:', fullUrl.substring(0, 200));
      return new NextResponse('Bad Request', { status: 400 });
    }

    if (hasXssAttempt(fullUrl)) {
      console.warn('[proxy] XSS attempt blocked:', fullUrl.substring(0, 200));
      return new NextResponse('Bad Request', { status: 400 });
    }
  ...
  if (isSensitivePath(pathname) && isMaliciousBot(userAgent)) {
    console.warn('[proxy] Blocked malicious bot:', userAgent.substring(0, 100));
