...
export function DxliveCredit({ className = '', variant = 'small' }: DxliveCreditProps) {
  ...
  if (variant === 'small') {
    return (
      ...
      >
        <div className="bg-gradient-to-r from-pink-600 to-purple-600 rounded px-3 py-2 text-white text-sm font-bold text-center min-w-[120px] hover:from-pink-500 hover:to-purple-500 transition-all">
  ...
  return (
    <a
      ...
      aria-label="DXLIVE ライブチャット"
    >
      <div className="bg-gradient-to-r from-pink-600 to-purple-600 rounded-lg p-4 text-white text-center hover:from-pink-500 hover:to-purple-500 transition-all">
        <div className="text-xs opacity-80 mb-1">アダルトライブチャット</div>
        <div className="text-xl font-bold">DXLIVE</div>
        <div className="text-xs mt-2 opacity-90">無料登録で$15相当のポイント付与</div>
      </div>
    </a>
  );
}
