...
function WatchlistPageClient({ locale }: WatchlistPageClientProps) {
  ...
  return (
    <div className="theme-body min-h-screen">
      <main id="watchlist" className="theme-bg min-h-screen">
        <div className="container mx-auto px-4 py-8">
            ...
            {count === 0 ? (
              ...
              <div className="space-y-3">
                {sortedItems.map((item) => (
                  <div
                    key={item.productId}
                    className="flex gap-4 rounded-lg border border-gray-700 bg-gray-800 p-4 transition-colors hover:border-gray-600"
                  >
                    {/* サムネイル */}
                    <div
                    ...
                    <div className="min-w-0 flex-1">
                      <h3
                        className="theme-text line-clamp-2 cursor-pointer font-medium transition-colors hover:text-blue-400"
                        onClick={() => handleProductClick(item.productId)}
                      >
                        {item.title}
                      </h3>
                      ...
                      {item.provider && (
                        <span className="mt-2 inline-block rounded bg-gray-700 px-2 py-0.5 text-xs text-gray-300">
                          {item.provider}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        ...
                        className="rounded px-3 py-1.5 text-sm text-gray-400 transition-colors hover:bg-red-900/20 hover:text-red-400"
                      >
                        {t.remove}
                      </button>
                    </div>
