...
export async function GET(request: NextRequest) {
  try {
    ...
    const productsData = await db
      ...
      .from(products)
      .where(isNumericIds ? inArray(products.id, numericIds) : inArray(products.normalizedProductId, idsInput));
    ...
    const commonTags = allTags.reduce((acc, set) => new Set([...acc].filter((x) => set.has(x))));

    const allPerformers = orderedProducts.map((p) => new Set(p.performers));
    const commonPerformers = allPerformers.reduce((acc, set) => new Set([...acc].filter((x) => set.has(x))));
