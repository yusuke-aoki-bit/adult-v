  ...
  return (
    <div className={`bg-white rounded-lg p-4 shadow ${className}`}>
      ...
      <div className="mb-4 p-3 bg-gray-50 rounded-lg text-sm">
        <p className="text-gray-500 mb-2">{t.basedOn}:</p>
        {performers.length > 0 && (
          <div className="flex items-center gap-2 mb-1">
            <span className="text-gray-400">{t.performers}:</span>
            <span className="text-gray-700">{performers.slice(0, 3).join(', ')}</span>
          </div>
        )}
        {tags.length > 0 && (
          <div className="flex items-center gap-2">
            <span className="text-gray-400">{t.tags}:</span>
            <span className="text-gray-700">{tags.slice(0, 5).join(', ')}</span>
          </div>
        )}
      </div>
            ...
            {results.map(product => (
              ...
              >
                <div className="relative rounded-lg overflow-hidden bg-gray-100 mb-2" style={{ aspectRatio: '2/3' }}>
                  ...
                  {/* Match badge */}
                  <div className="absolute top-1 right-1 px-1.5 py-0.5 bg-purple-500 text-white text-xs rounded font-medium">
                    {product.matchScore}%
                  </div>
                </div>
                <p className="text-sm text-gray-600 group-hover:text-gray-900 truncate transition-colors">
                  {product.title}
                </p>
              </Link>
            ))}
      ...
      {hasSearched && !isLoading && (
        <button
          ...
          className="mt-4 w-full py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-lg flex items-center justify-center gap-2 transition-colors"
        >
