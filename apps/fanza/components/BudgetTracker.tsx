    ...
    <div className={`bg-white rounded-lg p-6 shadow ${className}`}>
      ...
      {isEditing && (
        <div className="mb-4 p-3 bg-gray-50 rounded-lg">
          <label className="text-sm text-gray-500 mb-2 block">{t.setBudget}</label>
          <div className="flex gap-2">
            <input
              type="number"
              value={budgetInput}
              onChange={(e) => setBudgetInput(e.target.value)}
              className="flex-1 bg-white text-gray-900 px-3 py-2 rounded-lg border border-gray-300 focus:border-green-500 focus:outline-none"
              min="0"
              step="1000"
            />
            <button
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            ...
            <p className="text-lg font-bold text-gray-900">{formatPrice(stats.monthlyBudget)}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p className="text-xs text-gray-500 mb-1">{t.spent}</p>
        ...
        <div className={`flex items-center gap-2 p-2 rounded-lg ${
          ...
          {isOverBudget ? (
            <>
              ...
              <span className="text-sm text-red-600">{t.overBudget}</span>
            </>
          ) : (
            <>
              <CheckCircle2 className="w-4 h-4 text-green-500" />
              <span className="text-sm text-green-600">{t.onTrack}</span>
            </>
          )}
        </div>
        ...
        {stats.purchases.length > 0 && (
          <div className="border-t border-gray-200 pt-4">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-medium text-gray-500">{t.recentPurchases}</h4>
              {stats.purchases.length > 0 && (
                <button
