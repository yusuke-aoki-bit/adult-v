...
async function crawlSeesaaWiki(page: number): Promise<LookupEntry[]> {
  ...
  try {
    ...
    $('a[href*="/d/"]').each((_, el) => {
      ...
      if (productCodeMatch) {
        const productCode = productCodeMatch[1].toUpperCase();
        // ページ内容から出演者を取得する必要があるため、ここではエントリのみ記録
        entries.push({
          productCode,
          productCodeNormalized: normalizeProductCode(productCode),
          performerNames: [],
...
async function saveEntries(db: ReturnType<typeof getDb>, entries: LookupEntry[], source: string): Promise<{ inserted: number; updated: number }> {
  let inserted = 0;
  let updated = 0;
