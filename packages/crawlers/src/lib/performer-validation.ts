...
const INVALID_PATTERNS: RegExp[] = [
  ...
  // 記号のみ
  /^[!@#$%^&*()_+=\[\]{}|\\:";'<>?,./～・。、「」『』【】]+$/,
  // v8で追加 - 日付パターン（wikiからの誤検出防止）
  /^[0-9]{4}年[0-9]{1,2}月([0-9]{1,2}日)?$/,  // 2025年10月、2025年10月15日
  /^[0-9]{1,2}月[0-9]{1,2}日$/,  // 10月15日
  /^[0-9]{4}\/[0-9]{1,2}(\/[0-9]{1,2})?$/,  // 2025/10、2025/10/15
  /^[0-9]{4}-[0-9]{2}(-[0-9]{2})?$/,  // 2025-10、2025-10-15
  // v8で追加 - 画像枚数パターン
  /^[0-9]+枚$/,  // 116枚、200枚
  // v8で追加 - 時間・分数パターン
  /^[0-9]+分$/,  // 120分
...
const INVALID_NAMES: string[] = [
  ...
  'ヒロアキ', 'ケイスケ', 'ジャック',
  // v7で追加 - ASP名・サイト名（wikiからの誤検出防止）
  'MGS', 'mgs', 'Mgs', 'MGS動画',
  'FANZA', 'fanza', 'Fanza', 'FANZA動画',
  'DMM', 'dmm', 'Dmm', 'DMM.R18',
  'DUGA', 'duga', 'Duga',
  'ソクミル', 'SOKMIL', 'sokmil',
  'FC2', 'fc2', 'Fc2', 'FC2動画',
  'HEYZO', 'heyzo', 'Heyzo',
  'カリビアンコム', 'Caribbeancom', 'caribbeancom',
  '一本道', '1pondo', '1Pondo',
  'パコパコママ', 'pacopacomama',
  '天然むすめ', '10musume',
  'Tokyo-Hot', 'TOKYO-HOT', 'tokyo-hot',
  'Japanska', 'japanska', 'JAPANSKA',
  'b10f', 'B10F',
  'PRESTIGE', 'prestige', 'Prestige', 'プレステージ',
  'S1', 's1', 'エスワン',
  'MOODYZ', 'moodyz', 'Moodyz', 'ムーディーズ',
  'IDEA POCKET', 'Ideapocket', 'アイデアポケット',
  'E-BODY', 'e-body',
  'kira☆kira', 'KIRA☆KIRA',
  // v8で追加 - wikiからの誤検出（団体名・サークル名など）
  '日本エロ党大学', 'ラブラビッツ', '陳飛龍',
  'エロ党', '素人投稿', '素人盗撮',
  '無修正動画', '無修正', '修正版', '完全版', '限定版',
  '特別編', '総集編', 'スペシャル', 'ベスト版', 'コンプリート',
];
...
const SUSPICIOUS_PHRASES: RegExp[] = [
  ...
  // 助詞で終わる（タイトル断片の特徴）
  // 注: 「〜の」「〜や」で終わる演者名（水原みその、小那海あや）は有効
  // 漢字を含まず、かつ助詞で終わる長めの文字列のみ無効とする
  /^[ぁ-んァ-ヶー]{6,}[にをではがともへ]$/,

  // 「〜な〜」「〜の〜」パターン（かな/カナのみで7文字以上）
  // 注: 短い名前（「みその」など）は許可するため閾値を高めに
  /^[ぁ-んァ-ヶー]{3,}の[ぁ-んァ-ヶー]{3,}$/,
  /^[ぁ-んァ-ヶー]{3,}な[ぁ-んァ-ヶー]{3,}$/,
