...
export function parseDugaPageHtml(productId: string, html: string): DugaPageData {
  ...
  if (makerCodeMatch && makerCodeMatch[1]) {
  ...
  if (aggregateRatingBlock && aggregateRatingBlock[1]) {
    ...
    if (reviewCountMatch?.[1] && ratingValueMatch?.[1]) {
      result.aggregateRating = {
        reviewCount: parseInt(reviewCountMatch[1], 10),
        averageRating: parseFloat(ratingValueMatch[1]),
        worstRating: worstRatingMatch?.[1] ? parseInt(worstRatingMatch[1], 10) : 1,
        bestRating: bestRatingMatch?.[1] ? parseFloat(bestRatingMatch[1]) : 5,
      };
    }
  }
  ...
  for (const match of reviewMatches) {
    ...
    const reviewerId = reviewerIdMatch?.[1];
    const reviewerName = reviewerNameMatch?.[1]?.trim() ?? '';
    ...
    const helpfulYes = helpfulYesMatch?.[1] ? parseInt(helpfulYesMatch[1], 10) : 0;
    const helpfulNo = helpfulNoMatch?.[1] ? parseInt(helpfulNoMatch[1], 10) : 0;

    if (reviewId || title || content) {
      const review: DugaReview = {
        reviewId,
        rating,
        title,
        content,
