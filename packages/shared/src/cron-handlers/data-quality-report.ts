...
export function createDataQualityReportHandler(deps: DataQualityReportDeps) {
  ...
  return async function GET(request: NextRequest) {
    ...
    try {
      ...
      const linkingRate = parseFloat(linkSummary['linked_products']) / parseFloat(linkSummary['total_products']) * 100;
      if (linkingRate < 60) {
        recommendations.push(
      ...
      const ratingSummaryCount = (ratingSummaryStat.rows[0] as Record<string, number>)['products_with_rating_summary'] || 0;
      ...
      const report: DataQualityReport = {
        ...
        summary: {
          ...
          reviews: {
            ...
            byAsp: (reviewsByAsp.rows as Array<{
              asp_name: string;
              review_count: number;
              avg_rating: string;
        ...
        topPriorityPerformers: (topPriorityPerformers.rows as Array<{
          id: number;
          name: string;
        ...
        topPriorityProducts: (topPriorityProducts.rows as Array<{
          id: number;
        ...
        aspBreakdown: (aspBreakdown.rows as Array<{
          asp_name: string;
          product_count: number;
