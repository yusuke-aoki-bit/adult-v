...
export function useSceneInfo(productId: number) {
  ...
  const addScene = useCallback(
    (scene: Omit<SceneMarker, 'id' | 'votes' | 'createdAt'>) => {
      const allSceneInfo = getStoredSceneInfo();
      ...
      const sanitizedDescription = scene.description
        ? sanitizeText(scene.description, MAX_DESCRIPTION_LENGTH)
        : undefined;
  ...
  const voteScene = useCallback(
    (sceneId: string, upvote: boolean): 'voted' | 'toggled' | 'already_voted' | 'failed' => {
      const allSceneInfo = getStoredSceneInfo();
      const current = allSceneInfo[productId];
      ...
      const newScenes = current.scenes.map((s) =>
        s.id === sceneId ? { ...s, votes: Math.max(0, s.votes + voteChange) } : s,
      );
