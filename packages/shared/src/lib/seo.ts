...
export function generateAggregateOfferSchema(
  ...
  const saleEndDates = offers
    ...
    .filter((t) => !isNaN(t));
  const earliestSaleEnd =
    saleEndDates.length > 0 ? new Date(Math.min(...saleEndDates)).toISOString().split('T')[0] : undefined;
  ...
  return {
    ...
    offers: offers.map((offer) => {
      const offerSchema: Record<string, unknown> = {
        ...
        url: offer['url'],
        availability:
          offer['availability'] === 'OutOfStock' ? 'https://schema.org/OutOfStock' : 'https://schema.org/InStock',
        seller: {
          '@type': 'Organization',
  ...
  const faqsByLocale: Record<string, { question: string; answer: string }[]> = {
    ja: [
      ...
      },
      {
        ...
        answer:
          'DUGA、MGS（PRESTIGE・S1・Ideapocket等）、SOKMIL、カリビアンコム、カリビアンコムプレミアム、一本道、HEYZO、天然むすめ、パコパコママ、FC2コンテンツマーケット、Japanskaなど主要サイトに対応しています。',
      },
      {
        question: '無料で利用できますか？',
      ...
      },
      {
        ...
        answer:
          'トップページのセールセクション、または作品一覧ページの「セール中のみ」フィルターで確認できます。各配信サイトの最新セール情報をリアルタイムで収集しています。',
      },
    ],
    en: [
      {
      ...
      },
      {
        ...
        answer:
          'We support DUGA, MGS (PRESTIGE, S1, Ideapocket, etc.), SOKMIL, Caribbeancom, Caribbeancom Premium, 1Pondo, HEYZO, 10musume, Pacopacomama, FC2 Content Market, Japanska, and more.',
      },
      {
      ...
      },
      {
        ...
        answer:
          'You can find sales on the homepage sale section or by using the "On Sale Only" filter on the products page. We collect the latest sale information from each streaming site in real-time.',
      },
    ],
    zh: [
      {
      ...
      },
      {
        question: '支持哪些视频平台？',
        answer:
          '支持DUGA、MGS（PRESTIGE、S1、Ideapocket等）、SOKMIL、加勒比海、加勒比海Premium、一本道、HEYZO、天然娘、Pacopacomama、FC2内容市场、Japanska等主要平台。',
      },
      {
        question: '可以免费使用吗？',
    ...
    ko: [
      ...
      },
      {
        ...
        answer:
          'DUGA, MGS(PRESTIGE, S1, Ideapocket 등), SOKMIL, 카리비안컴, 카리비안컴 프리미엄, 일본도, HEYZO, 천연무스메, 파코파코마마, FC2 콘텐츠 마켓, Japanska 등 주요 사이트를 지원합니다.',
      },
      {
      ...
      },
      {
        ...
        answer:
          '홈페이지의 세일 섹션 또는 작품 목록 페이지의 "세일 중만" 필터에서 확인할 수 있습니다. 각 스트리밍 사이트의 최신 세일 정보를 실시간으로 수집합니다.',
      },
    ],
  };
...
export function generateOrganizationSchema(locale: string = 'ja') {
  ...
  return {
    ...
    knowsAbout: ['アダルト動画', 'AV女優', 'DUGA', 'MGS', 'SOKMIL', 'DTI', 'カリビアンコム', '一本道', 'HEYZO'],
    areaServed: {
      '@type': 'Country',
      name: 'Japan',
    },
  ...
  const faqsByLocale: Record<string, { question: string; answer: string }[]> = {
    ja: [
      ...
      ...(externalIds && externalIds.length > 0
        ? externalIds.map(({ aspName, originalProductId }) => ({
    ...
    ja: [
      {
        ...
        answer:
          '人気ジャンル、シチュエーション、プレイ内容、体型、コスチュームなど多彩なカテゴリから作品を探せます。各カテゴリは作品数順に並んでおり、人気のジャンルがすぐに見つかります。',
      },
      {
        question: '複数のジャンルを組み合わせて検索できますか？',
        answer:
          'はい、作品一覧ページでは「含む」と「除外」フィルターを使って、複数のジャンルを組み合わせた検索が可能です。お好みの条件で絞り込むことができます。',
      },
      {
        question: 'どのカテゴリが一番人気ですか？',
        answer:
          'カテゴリ一覧ページでは各ジャンルの作品数が表示されています。作品数が多いジャンルほど人気があり、豊富なコンテンツから選ぶことができます。',
      },
    ],
    en: [
      {
        ...
        answer:
          'You can search for products from various categories including popular genres, situations, play types, body types, and costumes. Categories are sorted by the number of products, making it easy to find popular genres.',
      },
      {
        question: 'Can I combine multiple genres in my search?',
        answer:
          'Yes, on the product listing page, you can use "Include" and "Exclude" filters to combine multiple genres. This allows you to narrow down your search to match your preferences.',
      },
      {
        question: 'Which categories are most popular?',
        answer:
          'The category listing page shows the number of products for each genre. Genres with more products are more popular and offer a wider selection of content to choose from.',
      },
    ],
    zh: [
      {
        ...
        answer:
          '您可以从人气类型、场景、玩法、体型、服装等多种分类中搜索作品。各分类按作品数量排序，可以快速找到热门类型。',
      },
      {
    ...
    ko: [
      {
        ...
        answer:
          '인기 장르, 상황, 플레이 유형, 체형, 의상 등 다양한 카테고리에서 작품을 검색할 수 있습니다. 각 카테고리는 작품 수 순으로 정렬되어 인기 장르를 쉽게 찾을 수 있습니다.',
      },
      {
        question: '여러 장르를 조합하여 검색할 수 있나요?',
        answer:
          '네, 작품 목록 페이지에서 "포함" 및 "제외" 필터를 사용하여 여러 장르를 조합한 검색이 가능합니다. 원하는 조건으로 좁힐 수 있습니다.',
      },
      {
        question: '어떤 카테고리가 가장 인기 있나요?',
        answer:
          '카테고리 목록 페이지에서 각 장르의 작품 수를 확인할 수 있습니다. 작품 수가 많은 장르일수록 인기가 있으며, 풍부한 콘텐츠에서 선택할 수 있습니다.',
      },
    ],
  };
...
export function generateCriticReviewSchema(
  ...
  const reviewSummary =
    options?.['summary'] || (reviewBody.length > 150 ? reviewBody.substring(0, 147) + '...' : reviewBody);
