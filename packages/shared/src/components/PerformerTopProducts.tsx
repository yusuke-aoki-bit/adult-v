...
export default function PerformerTopProducts({
  ...
  return (
    <div
      className={`mt-8 rounded-lg p-6 ${isDark ? 'bg-linear-to-br from-amber-900/30 to-orange-900/30' : 'bg-linear-to-br from-amber-50 to-orange-50'}`}
    >
      <h2 className={`mb-2 flex items-center gap-2 text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
        <svg
          className={`h-6 w-6 ${isDark ? 'text-yellow-400' : 'text-yellow-600'}`}
          fill="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      <p className={`mb-4 text-sm ${isDark ? 'text-amber-300' : 'text-amber-700'}`}>{translations.description}</p>
        ...
        {products.map((product) => {
          ...
          return (
            ...
            >
              {/* ランキングバッジ */}
              <div
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${rankStyle.bg} ${rankStyle.text}`}
              >
                {product['rank']}
              </div>

              {/* サムネイル */}
              <div className={`relative h-16 w-24 shrink-0 overflow-hidden rounded ring-2 ${rankStyle.border}`}>
                <Image
              ...
              <div className="min-w-0 flex-1">
                <h3
                  className={`line-clamp-1 text-sm font-medium transition-colors ${
                ...
                <div
                  className={`mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'}`}
                >
                  {/* 評価 */}
                  {product['rating'] && product['rating'] > 0 && (
                    <span className="flex items-center gap-1">
                      <svg
                        className={`h-3.5 w-3.5 ${isDark ? 'text-yellow-400' : 'text-yellow-500'}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
