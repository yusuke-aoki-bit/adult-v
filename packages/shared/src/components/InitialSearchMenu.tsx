...
function InitialSearchMenu({
  ...
  const activeColor = primaryColor === 'rose' ? 'bg-rose-600' : 'bg-pink-500';
        ...
        >
          ...
          {hoveredGroup === group && (
            <div
              ...
              onMouseLeave={() => setHoveredGroup(null)}
            >
              <div className={`${isDark ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-200'} border rounded shadow-lg p-2 flex gap-1`}>
                {chars.map((char) => (
                  <Link
                    key={char}
                    href={buildUrl(char)}
