...
export function AiSearchBar({
  ...
  return (
    <div className="relative">
      <div className={`relative flex items-center gap-2 px-4 py-3 rounded-xl border ${themeStyles.container} shadow-lg`}>
        {/* AI Badge */}
        <span className={`shrink-0 px-2 py-0.5 text-xs font-semibold rounded-full ${themeStyles.badge}`}>
        ...
        {isLoading ? (
          <div className="flex items-center gap-2">
            <div className={`animate-spin rounded-full h-4 w-4 border-b-2 ${themeStyles.spinner}`} />
          ...
          <>
            {query && (
              <button
                type="button"
                onClick={() => setQuery('')}
                className={`p-1 rounded ${theme === 'dark' ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-gray-700'}`}
              >
      ...
      {/* Example queries */}
      {showExamples && !query && (
        <div className={`absolute top-full left-0 right-0 mt-2 p-3 rounded-lg border ${
          ...
          <div className="flex flex-wrap gap-2">
            {t.examples.map((example, i) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded-full transition-colors ${themeStyles.examples}`}
              >
                {example}
              </button>
            ))}
