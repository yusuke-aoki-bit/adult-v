  ...
  return async function POST(request: NextRequest) {
    try {
      ...
      if (!history || !Array.isArray(history) || history.length < 5) {
        return NextResponse.json({ success: false, profile: null, message: 'もう少し作品を閲覧するとプロファイルが生成されます（5件以上必要）' });
      }
      ...
      const [productData, performerData, tagData] = await Promise.all([
        db.select({ id: deps.products.id, title: deps.products.title, releaseDate: deps.products.releaseDate })
          .from(deps.products).where(deps.inArray(deps.products.id, productIds)),
        db.select({ productId: deps.productPerformers.productId, performerName: deps.performers.name })
        ...
        db.select({ productId: deps.productTags.productId, tagName: deps.tags.name, tagCategory: deps.tags.category })
      ...
      const uniqueGenres = new Set(tagData.filter((t: any) => t.tagCategory === 'genre' || !t.tagCategory).map((t: any) => t.tagName));
      ...
      const availableGenresResult = await db.select({ name: deps.tags.name }).from(deps.tags).where(deps.eq(deps.tags.category, 'genre')).limit(50);
      const availableGenres = availableGenresResult.map((r: any) => r.name);

      const profile = await deps.generateUserPreferenceProfile({
        recentProducts,
        viewingStats: { totalViewed: history.length, uniquePerformers: uniquePerformers.size, uniqueGenres: uniqueGenres.size },
        availableGenres,
      });
      ...
      return NextResponse.json({
        ...
        stats: { totalViewed: history.length, uniquePerformers: uniquePerformers.size, uniqueGenres: uniqueGenres.size, topPerformers: [...uniquePerformers].slice(0, 5), topGenres: [...uniqueGenres].slice(0, 5) },
        message: 'プロファイルを生成しました',
      });
    } catch (error) {
      console.error('[User Profile API] Error:', error);
      return NextResponse.json({ success: false, fallback: true, profile: null, message: 'プロファイル生成に失敗しました' });
    }
  };
}
