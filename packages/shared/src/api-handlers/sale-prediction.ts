...
export interface SalePredictionHandlerDeps {
  getDb: () => DbType;
  sql: SqlType;
}
...
export function createSalePredictionHandler(deps: SalePredictionHandlerDeps) {
  return async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
      ...
      // セール日を月別に集計
      const saleDates = saleHistory.map((s: { recorded_at: unknown }) => new Date(s.recorded_at as string));
      const monthCounts: { [key: number]: number } = {};
      saleDates.forEach((date: Date) => {
      ...
      const sortedDates = saleDates.sort((a: Date, b: Date) => a.getTime() - b.getTime());
      let totalDuration = 0;
      let saleCount = 0;
