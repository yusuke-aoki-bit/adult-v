...
async function main() {
  ...
  // Also check uncensored_products
  const uncensoredCount = await db.execute(sql`
    SELECT COUNT(*) as count FROM uncensored_products
  `);

  console.log(`📦 Uncensored products archived: ${uncensoredCount.rows[0].count}\n`);
}
