/**
 * Sokmil商品に出演者を紐付けるバックフィルスクリプト
 *
 * sokmil_raw_responsesテーブルから出演者情報を抽出して紐付け
 */
...
async function main() {
  ...
  console.log('=== Sokmil 出演者紐付けバックフィル ===\n');
  console.log(`Limit: ${limit}, Batch: ${batch}\n`);

  // 方法: sokmil_raw_responsesから出演者情報を抽出
  console.log('🔍 未紐付きSokmil商品を検索中...\n');

  // sokmil_raw_responsesから出演者情報を持つレコードを取得
  const rawDataWithPerformers = await db.execute(sql`
    SELECT
      ...
      ps.product_id,
      p.title
    FROM sokmil_raw_responses srr
    JOIN product_sources ps ON ps.original_product_id = srr.item_id AND ps.asp_name = 'ソクミル'
  ...
  for (const row of rawDataWithPerformers.rows as any[]) {
    try {
      ...
      // 出演者情報を抽出（Sokmil APIの形式に対応）
      let performerNames: string[] = [];

      // actors フィールドを確認
      if (rawJson.actors && Array.isArray(rawJson.actors)) {
        performerNames = rawJson.actors
          .map((a: any) => a.name || a.actorName || a)
          .filter((n: any) => typeof n === 'string' && n.length > 0);
      }

      // performers フィールドも確認（別のAPI形式）
      if (performerNames.length === 0 && rawJson.performers && Array.isArray(rawJson.performers)) {
