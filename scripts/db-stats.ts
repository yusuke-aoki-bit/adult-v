...
import {
  ...
  wikiCrawlData,
} from '../packages/crawlers/src/lib/db/schema';
...
async function main() {
  ...
  const [fullProfile] = await db
    .select({ count: count() })
    .from(performers)
    .where(and(isNotNull(performers.profileImageUrl), isNotNull(performers.height), isNotNull(performers.birthday)));
  ...
  // 外部ID統計
  console.log('\n🔗 外部ID連携:');
  const externalIdCounts = await db
    .select({
  ...
  // 処理済み/未処理
  const [processedWiki] = await db
    .select({ count: count() })
    .from(wikiCrawlData)
    .where(isNotNull(wikiCrawlData.processedAt));
  const [totalWiki] = await db.select({ count: count() }).from(wikiCrawlData);
  const processedRate = totalWiki.count
    ? (((processedWiki.count as number) / (totalWiki.count as number)) * 100).toFixed(1)
    : '0';
