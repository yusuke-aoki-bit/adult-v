      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          {data.tagStats && data.tagStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Tag Statistics by Category</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                      <th className="pb-3">Category</th>
                      <th className="pb-3 text-right">Tags</th>
                  ...
                  <tbody>
                    {data.tagStats.map((s) => (
                      <tr key={s.category || 'uncategorized'} className="border-b border-gray-700/50">
                        <td className="py-2">{s.category || '(uncategorized)'}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.tag_count)}</td>
                        <td className="py-2 text-right font-mono text-blue-400">{formatNumber(s.product_tag_links)}</td>
                        <td className="py-2 text-right font-mono text-green-400">{formatNumber(s.products_with_tag)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {data.topTags && data.topTags.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Top 20 Tags</h2>
              <div className="max-h-80 overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-800">
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                  ...
                  <tbody>
                    {data.topTags.map((t, i) => (
                      <tr key={`${t.name}-${i}`} className="border-b border-gray-700/50">
                        <td className="py-2">{t.name}</td>
                        <td className="py-2 text-gray-400">{t.category || '-'}</td>
                        <td className="py-2 text-right font-mono text-blue-400">{formatNumber(t.product_count)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          {data.wikiCrawlStats && data.wikiCrawlStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Wiki Crawl Data</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                      ...
                      <th className="pb-3 text-right">Products</th>
                      <th className="pb-3 text-right">Performers</th>
                      <th className="pb-3 text-right">Processed</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.wikiCrawlStats.map((s) => (
                      <tr key={s.source} className="border-b border-gray-700/50">
                        <td className="py-2">{s.source}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.total_records)}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.unique_products)}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.unique_performers)}</td>
                        <td className="py-2 text-right font-mono text-green-400">{formatNumber(s.processed_count)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {data.wikiIndexStats && data.wikiIndexStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Wiki Performer Index</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                  ...
                  <tbody>
                    {data.wikiIndexStats.map((s) => (
                      <tr key={s.source} className="border-b border-gray-700/50">
                        <td className="py-2">{s.source}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.total_records)}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.unique_product_codes)}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.unique_performers)}</td>
                        <td className="py-2 text-right font-mono text-green-400">{formatNumber(s.verified_count)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {/* Product Media Statistics */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          {data.productImageStats && data.productImageStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Product Images by Type/ASP</h2>
              <div className="max-h-80 overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-800">
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                      ...
                      <th className="pb-3">ASP</th>
                      <th className="pb-3 text-right">Images</th>
                      <th className="pb-3 text-right">Products</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.productImageStats.map((s, i) => (
                      <tr key={`${s.image_type}-${s.asp_name}-${i}`} className="border-b border-gray-700/50">
                        <td className="py-2">{s.image_type}</td>
                        <td className="py-2 text-gray-400">{s.asp_name}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.count)}</td>
                        <td className="py-2 text-right font-mono text-blue-400">{formatNumber(s.products)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {data.productVideoTypeStats && data.productVideoTypeStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Product Videos by Type/Quality</h2>
              <div className="max-h-80 overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-800">
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                  ...
                  <tbody>
                    {data.productVideoTypeStats.map((s, i) => (
                      <tr key={`${s.video_type}-${s.quality}-${s.asp_name}-${i}`} className="border-b border-gray-700/50">
                        <td className="py-2">{s.video_type || '-'}</td>
                        <td className="py-2 text-gray-400">{s.quality || '-'}</td>
                        <td className="py-2 text-gray-400">{s.asp_name}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.count)}</td>
                      </tr>
                    ))}
        ...
        {/* Performer Detail Statistics */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          {data.performerImageStats && data.performerImageStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Performer Images</h2>
                ...
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-800">
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                      <th className="pb-3">Source</th>
                      <th className="pb-3 text-right">Images</th>
                      <th className="pb-3 text-right">Primary</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.performerImageStats.map((s, i) => (
                      <tr key={`${s.source}-${s.image_type}-${i}`} className="border-b border-gray-700/50">
                        <td className="py-2 text-xs">{s.source}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.count)}</td>
                        <td className="py-2 text-right font-mono text-green-400">{formatNumber(s.primary_count)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {data.performerAliasStats && data.performerAliasStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Performer Aliases</h2>
              <div className="max-h-60 overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-800">
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                      <th className="pb-3">Source</th>
                      <th className="pb-3 text-right">Aliases</th>
                      <th className="pb-3 text-right">Primary</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.performerAliasStats.map((s, i) => (
                      <tr key={`${s.source}-${i}`} className="border-b border-gray-700/50">
                        <td className="py-2 text-xs">{s.source || '(none)'}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.count)}</td>
                        <td className="py-2 text-right font-mono text-green-400">{formatNumber(s.primary_count)}</td>
                      </tr>
                    ))}
          ...
          {data.performerExternalIdStats && data.performerExternalIdStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              <div className="max-h-60 overflow-y-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-800">
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                      <th className="pb-3">Provider</th>
                      <th className="pb-3 text-right">IDs</th>
                      <th className="pb-3 text-right">Performers</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.performerExternalIdStats.map((s, i) => (
                      <tr key={`${s.provider}-${i}`} className="border-b border-gray-700/50">
                        <td className="py-2">{s.provider}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.count)}</td>
                        <td className="py-2 text-right font-mono text-blue-400">{formatNumber(s.performers)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {/* Sales & Reviews Statistics */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          {data.salesStats && data.salesStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4">Sales/Discounts</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-gray-400 border-b border-gray-700">
                  ...
                  <tbody>
                    {data.salesStats.map((s) => (
                      <tr key={s.sale_type} className="border-b border-gray-700/50">
                        <td className="py-2">{s.sale_type || 'general'}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.count)}</td>
                        <td className="py-2 text-right font-mono text-green-400">{formatNumber(s.active_count)}</td>
                        <td className="py-2 text-right font-mono">{s.avg_discount ? `${parseFloat(s.avg_discount).toFixed(0)}%` : '-'}</td>
                        <td className="py-2 text-right font-mono text-xs">
                          {s.min_discount && s.max_discount ? `${s.min_discount}-${s.max_discount}%` : '-'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {data.reviewStats && data.reviewStats.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {data.reviewStats.map((s) => (
                      <tr key={s.asp_name} className="border-b border-gray-700/50">
                        <td className="py-2">{s.asp_name}</td>
                        <td className="py-2 text-right font-mono">{formatNumber(s.review_count)}</td>
                        <td className="py-2 text-right font-mono text-blue-400">{formatNumber(s.products_reviewed)}</td>
                        <td className="py-2 text-right font-mono text-yellow-400">
                          {s.avg_rating ? parseFloat(s.avg_rating).toFixed(2) : '-'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {/* Price Statistics */}
        {data.priceStats && data.priceStats.length > 0 && (
          <div className="bg-gray-800 rounded-lg p-6 mb-8">
            <h2 className="text-xl font-semibold mb-4">Price Statistics by ASP/Currency</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-gray-400 border-b border-gray-700">
                ...
                <tbody>
                  {data.priceStats.map((s, i) => (
                    <tr key={`${s.asp_name}-${s.currency}-${i}`} className="border-b border-gray-700/50">
                      <td className="py-2">{s.asp_name}</td>
                      <td className="py-2 text-gray-400">{s.currency}</td>
                      <td className="py-2 text-right font-mono">{formatNumber(s.count)}</td>
                      <td className="py-2 text-right font-mono text-green-400">{formatNumber(s.min_price)}</td>
                      <td className="py-2 text-right font-mono text-blue-400">{parseInt(s.avg_price).toLocaleString()}</td>
                      <td className="py-2 text-right font-mono text-red-400">{formatNumber(s.max_price)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
